...
const handleAudit = (id) => {
  ElMessageBox.confirm('确认审核通过该入库单？', '提示', { type: 'warning' }).then(async () => {
...
const handleComplete = (id) => {
  ElMessageBox.confirm('确认执行入库操作？物资将入库到仓库中。', '提示', { type: 'warning' }).then(async () => {
    ...
    if (res.data.code === 200) { ElMessage.success('入库成功'); loadData() }
  }).catch(() => {})
}

const handleDelete = (id) => {
  ElMessageBox.confirm('确认删除该入库单？', '提示', { type: 'warning' }).then(async () => {
