    ...
    <el-card class="search-card">
      <el-form :inline="true" :model="query">
        ...
        <el-form-item label="客户">
          <el-select v-model="query.customerId" placeholder="全部" clearable filterable>
...
const handleAudit = (id) => {
  ElMessageBox.confirm('确认审核通过该出库单？', '提示', { type: 'warning' }).then(async () => {
...
const handleComplete = (id) => {
  ElMessageBox.confirm('确认执行出库操作？物资将从仓库中扣减。', '提示', { type: 'warning' }).then(async () => {
    ...
    if (res.data.code === 200) { ElMessage.success('出库成功'); loadData() }
  }).catch(() => {})
}

const handleDelete = (id) => {
  ElMessageBox.confirm('确认删除该出库单？', '提示', { type: 'warning' }).then(async () => {
