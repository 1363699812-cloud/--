<template>
  <div class="page-container">
    <el-card class="search-card">
      <el-form :inline="true" :model="query">
        <el-form-item label="供应商名称">
          <el-input v-model="query.name" placeholder="请输入供应商名称" clearable @keyup.enter="handleSearch" />
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog v-model="dialogVisible" :title="form.id ? '编辑供应商' : '新增供应商'" width="550px" destroy-on-close>
...
import { getSupplierList, saveSupplier, updateSupplier, deleteSupplier } from '@/api'
...
const handleToggleStatus = async (row) => {
  ...
  const res = await updateSupplier(row.id, { status: newStatus })
